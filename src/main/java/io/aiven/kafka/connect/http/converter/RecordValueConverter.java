    ...
    private static void validateConvertibility(
        ...
        if (!isConvertibleType) {
            throw new DataException(
                String.format(
                    "Record value must be a String, a Schema Struct or implement one of "
                        + "[`java.util.Map.Entry`, `java.util.Map`] interfaces, but %s is given",
                    recordClazz));
        }
        if (!directlyConvertible && convertibleByImplementedTypes.size() > 1) {
            ...
            throw new DataException(
                String.format(
                    "Record value must be only one of String, Schema Struct or implement one of "
                        + "[`java.util.Map.Entry`, `java.util.Map`] interfaces,"
                        + " but %s matches multiple types: %s",
